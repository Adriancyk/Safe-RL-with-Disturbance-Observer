...
class UnicycleEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        # self.state_nom = self.dt * (self.get_f(self.state) + self.get_g(self.state) @ action) + self.state
        self.uncertainty = -0.1 * 4 * self.get_g(self.state) @ np.array([np.cos(self.state[2]),  0])
        ...
        #* np.random.multivariate_normal(self.disturb_mean, self.disturb_covar, 1).squeeze()
