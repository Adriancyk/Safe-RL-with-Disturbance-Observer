    ...
    def get_cbf_qp_constraints(self, state_batch, action_batch, mean_pred_batch, sigma_pred_batch, use_L1=False):
        ...
        if self.env.dynamics_mode == 'Unicycle':
            ...
            for i in range(num_cbfs):
                temp = (ps_hzds[:, i, 0] - hazards_locations[i, 0]) ** 2 + (ps_hzds[:, i, 1] - hazards_locations[i, 1]) ** 2 - collision_radius ** 2
                temp = temp.cpu().detach().numpy()
                if temp < 0:
                    h_value = 1
                    break
            hs = 0.5 * (torch.sum((ps_hzds - hazards_locations.view(1, num_cbfs, -1)) ** 2, axis=2) - collision_radius ** 2)  # 1/2 * (||x - x_obs||^2 - r^2)
            # temp = hs.cpu().detach().numpy()
            # for i in temp[0]:
            #     if i < 0:
            #         h_value = 1
            #         break
            ...
            h[:, :num_cbfs] = gamma_b * (hs ** 3) + (torch.bmm(dhdps, f_ps + mu_ps) - 0*torch.bmm(torch.abs(dhdps), sigma_ps) + torch.bmm(torch.bmm(dhdps, g_ps), action_batch)).squeeze(-1)
            ...
            if self.use_L1:
                ...
                h[:, 0] = Lffh + (gamma_b + gamma_b) * dh_cbf + gamma_b * gamma_b * h_cbf + torch.bmm(Lgfh, action_batch).squeeze() + LDffh #- L1_penalty
            else:
                h[:, 0] = Lffh + (gamma_b + gamma_b) * dh_cbf + gamma_b * gamma_b * h_cbf + torch.bmm(Lgfh, action_batch).squeeze() - LDffh
            
            G[:, 0, 0] = -Lgfh.squeeze()[0]
            G[:, 0, 1] = -Lgfh.squeeze()[1]
            G[:, :self.num_cbfs, n_u] = -2e2  # for slack
            ineq_constraint_counter += self.num_cbfs
